...
final class NurtureAlg[F[_]](config: VCSCfg)(implicit
  ...
  def applyNewUpdate(data: UpdateData): F[ProcessResult] =
    gitAlg.returnToCurrentBranch(data.repo) {
      ...
      editAlg.applyUpdate(data.repoData, data.update, data.updateBranch, createBranch).flatMap {
        edits =>
  ...
  def mergeAndApplyAgain(data: UpdateData): F[ProcessResult] =
    for {
      ...
      edits <- editAlg.applyUpdate(data.repoData, data.update, data.updateBranch)
