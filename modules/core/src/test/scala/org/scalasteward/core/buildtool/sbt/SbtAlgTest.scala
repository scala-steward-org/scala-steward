...
class SbtAlgTest extends FunSuite {
  test("addGlobalPlugins") {
    ...
    val expected = MockState.empty.copy(
      trace = Vector(
        ...
        Cmd("write", "/tmp/scala-steward/.sbt/0.13/plugins/StewardPlugin.scala"),
        Cmd("write", "/tmp/scala-steward/.sbt/1.0/plugins/StewardPlugin.scala"),
        Cmd("fa"),
        Cmd("rm", "-rf", "/tmp/scala-steward/.sbt/1.0/plugins/StewardPlugin.scala"),
        Cmd("rm", "-rf", "/tmp/scala-steward/.sbt/0.13/plugins/StewardPlugin.scala")
      )
    )
  ...
  test("runMigrations") {
    ...
    val expected = MockState.empty.copy(
      trace = Vector(
        Cmd("write", "/tmp/scala-steward/.sbt/0.13/plugins/scala-steward-scalafix.sbt"),
        Cmd("write", "/tmp/scala-steward/.sbt/1.0/plugins/scala-steward-scalafix.sbt"),
        Cmd(
          repoDir.toString,
          "firejail",
          "--quiet",
        ...
        Cmd("rm", "-rf", "/tmp/scala-steward/.sbt/1.0/plugins/scala-steward-scalafix.sbt"),
        Cmd("rm", "-rf", "/tmp/scala-steward/.sbt/0.13/plugins/scala-steward-scalafix.sbt")
      )
    )
    assertEquals(state, expected)
  }
