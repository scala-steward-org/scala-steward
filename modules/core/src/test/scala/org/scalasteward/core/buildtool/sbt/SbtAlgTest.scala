...
class SbtAlgTest extends FunSuite {
  ...
  test("sbt 2.0") {
    val repo = Repo("sbt-alg", "test-2-0")
    val buildRoot = BuildRoot(repo, ".")
    val repoDir = workspaceAlg.repoDir(repo).unsafeRunSync()
    val initial = MockState.empty
  ...
  test("sbt 2.1") {
    val repo = Repo("sbt-alg", "test-2-1")
    val buildRoot = BuildRoot(repo, ".")
    val repoDir = workspaceAlg.repoDir(repo).unsafeRunSync()
    val initial = MockState.empty
      .addFiles(repoDir / "project" / "build.properties" -> "sbt.version=2.1.0-M1")
      .unsafeRunSync()
    val state = sbtAlg.getDependencies(buildRoot).runS(initial).unsafeRunSync()
    val expected = initial.copy(
      trace = Vector(
        ...
        Cmd("test", "-d", s"$repoDir/project"),
        Cmd("test", "-d", s"$repoDir/project/project"),
        Cmd("read", "classpath:StewardPlugin_2_1_0.scala"),
        Cmd("write", s"$repoDir/project/scala-steward-StewardPlugin_2_1_0.scala"),
        Cmd("write", s"$repoDir/project/project/scala-steward-StewardPlugin_2_1_0.scala"),
        Cmd.execSandboxed(
          repoDir,
          "sbt",
          "--server",
        ...
        Cmd("rm", "-rf", s"$repoDir/project/project/scala-steward-StewardPlugin_2_1_0.scala"),
        Cmd("rm", "-rf", s"$repoDir/project/scala-steward-StewardPlugin_2_1_0.scala")
      )
    )
    assertEquals(state, expected)
  }
