...
class BitbucketApiAlgTest extends CatsEffectSuite with Http4sDsl[MockEff] {
  ...
  private val forgeCfg =
    config.forgeCfg.copy(tpe = ForgeType.Bitbucket, apiHost = uri"https://bitbucket.org")
  private val bitbucketCfg = BitbucketCfg(useDefaultReviewers = true)
  private val bitbucketApiAlg =
    ForgeSelection.forgeApiAlg[MockEff](forgeCfg, bitbucketCfg, noAuth.authenticateApi)
