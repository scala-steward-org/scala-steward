...
class MavenParserTest extends AnyFunSuite with Matchers {
  ...
  val pluginUpdates: List[String] = """[INFO] Scanning for projects...
  ...
  test("parse `mvn versions:display-plugin-updates`") {
    MavenAlg.parseUpdates(pluginUpdates) shouldBe List(
      RawUpdate(
        Dependency(GroupId("com.twilio.maven.plugins"), ArtifactId("guardrail-twilio-maven-plugin","guardrail-twilio-maven-plugin"),"0.53.1",None,None,None), NonEmptyList.one("4.jsterlinginstanttest")
      ),
      RawUpdate(
        Dependency(GroupId("net.alchim31.maven"), ArtifactId("scala-maven-plugin","scala-maven-plugin"),"3.3.2",None,None,None), NonEmptyList.one("4.3.0")
      )
    )
  }

  test("parse mvn dependency:list into dependencies") {
    MavenAlg.parseDependencies(dependencies).head shouldBe Dependency(
      GroupId("com.twilio"), ArtifactId("scala-service-http-server","scala-service-http-server_2.12"), "0.52.0"
    )
  }

  test("parse mvn display-dependency-updates into updates") {
    MavenAlg.parseUpdates(updates).head shouldBe RawUpdate(
      Dependency(GroupId("io.kamon"), ArtifactId("kamon-core", "kamon-core_2.12"), "0.6.5", sbtVersion = None, scalaVersion = None, configurations = None),
  ...
  test("merge minor and major dependency updates") {
    val minorUpdate1 = RawUpdate(
      Dependency(
        ...
        "0.6.5", sbtVersion = None, scalaVersion = None, configurations = None),
    ...
    MavenAlg.groupNewerVersions(Nel.of(minorUpdate1, minorUpdate2, majorUpdate1).toList).toSet shouldBe Set(
      RawUpdate(Dependency(GroupId("io.kamon"), ArtifactId("kamon-datadog", "kamon-datadog_2.12"), "0.6.5", None, None, None), NonEmptyList.one("0.6.8")),
      RawUpdate(Dependency(GroupId("io.kamon"), ArtifactId("kamon-core", "kamon-core_2.12"), "0.6.5", None, None, None), NonEmptyList.of("0.6.8", "2.0.2"))
    )
  }
